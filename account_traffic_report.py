...
class AccountTrafficReport:
	...
	def __init__(self, account_data):
		"""Contructor for AccountTrafficReport. Sets object attributes"""
	...
	def get_average_of_data(self):
		"""Calculates average score from list of scores"""
	...
	def get_highest_of_data(self):
		"""Calculates highest score from list and the number of occurences"""

		return [max(self.account_data), self.account_data.count(max(self.account_data))]

	def get_lowest_of_data(self):
		"""Calculates lowest score from list and the number of occurences"""
		return [min(self.account_data), self.account_data.count(min(self.account_data))]

	def get_server_recommendation(self):
		"""Parses list of scores and compares to our server metrics to make plan recommendation"""
		...
		for key, value in score_key.items():
			...
			if self.account_high[0] >= value[0] and self.account_high[0] <= value[1] or self.account_high[0] > 80:
